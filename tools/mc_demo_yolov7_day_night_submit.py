...
def day_classifier(im0s):
    ...
    if avg_brightness > 105 and dark_prop < 0.25:
        return 1
    else:
        return 0
...
def detect(save_img=False):
    ...
    frame_rate =30
    tracker = BoTSORT_ADJUST(opt, frame_rate,prev_cnt) 
    # Run inference
    if device.type != 'cpu':
    ...
    for path, img, im0s, vid_cap in tqdm(dataset, desc=f'tracking {opt.name}'):
        ...
        #camera id
        cam_id = int(path.split("\\")[-1].strip().split("_")[0])
        if cam_id - camera_id > 0:
            is_change_camera = 1
        else:
            is_change_camera = 0
        camera_id = cam_id
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            online_targets = tracker.update(detections_car, im0, frameID, day,opt,is_change_camera)
            #add
            for detect in detections:
                cls = detect[5]
                label = f' { names[int(cls)] }'
...
if __name__ == '__main__':
    ...
    # tracking args
    parser.add_argument("--track_high_thresh", type=float, default=0.18, help="tracking confidence threshold") #0.4
    ...
    parser.add_argument("--match_thresh_night", type=float, default=0.5, help="matching threshold for tracking") #default=0.35
    ...
    parser.add_argument("--match_thresh", type=float, default=0.25, help="matching threshold for tracking") #default=0.35
